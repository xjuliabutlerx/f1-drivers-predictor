...
- v2 (a curated gallery of the strongest individual runs from a width/activation-function experiment, not one model per holdout pair — 2 of the 3 share the same held-out seasons, and 2024-2025 has no v2 representative at all): `prost_model.pt` (2022-2023, LeakyReLU, rho 0.9968, max error 1.0 — the standout of the whole project so far), `schumacher_model.pt` (2020-2021, ReLU, rho 0.9938), `senna_model.pt` (2022-2023, GELU, rho 0.9917). Each v2 keeper has its own dedicated `f1_drivers_rank_classifier_{name}.py` (not the shared `f1_drivers_rank_classifier.py`, which keeps changing across experiments) so its specific architecture/activation combination stays loadable.

Heatmap and SHAP output filenames are kept in sync with these names (v2 has no SHAP analysis yet).
