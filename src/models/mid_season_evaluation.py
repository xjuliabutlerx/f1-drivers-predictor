...
def resolve_classifier_class(version: int, model_path: str):
    ...
    the loaded weights through the wrong nonlinearity, producing systematically wrong scores with no
    error at all. Falls back to the shared file for models that don't have a dedicated one (e.g. all
    of v1, which has never diverged from a single shared architecture)."""
