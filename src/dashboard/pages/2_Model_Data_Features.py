...
st.write("Here is an overview of all 57 data features used by v1 and v2 (47 numeric features plus 10 one-hot `TeamId` columns). v2 changed model architecture, not the feature set, so this list is identical across both generations. They're divided into the same 3 sections as `f1-constructors-predictor`: raw features pulled directly from `fastf1` race data, calculated features (counts, sums, averages, rates) produced during preprocessing, and interaction features that combine multiple calculated statistics during feature engineering.")

st.write("A v3 experiment tried dropping the SHAP-flagged low-value features noted below (the individual `TeamId_*` columns, `HasQualifyingData`, and the DNF-cause breakdown), but the result was inconclusive. See [Model Development & Training](/Model_Development_&_Training) for the full comparison. Since v2 was ultimately kept as the final generation, this feature list never actually changed from v1.")

st.subheader("Raw / Identifying Features")
st.write("*Note: unlike the constructors project, `fastf1`'s race data is already at driver granularity, so no per-team consolidation was needed here. `DriverId` is deliberately never used as a model feature, since one-hot encoding driver identity would let the model memorize specific drivers' outcomes instead of learning transferable performance signal, and would produce a useless all-zero vector for any driver not seen in training.*")
...
- **DriverPointsShareOfTeam**: This driver's share of their own team's combined points, separating individual performance from car strength.
...
st.write("Per SHAP analysis, the individual `TeamId_*` columns are consistently among the least influential features across all 3 v1 models, likely because `ProjectedSeasonTotalPoints`/`TeamProjectedSeasonTotalPoints` already capture most of what team identity would otherwise signal (car strength). A v3 experiment tried dropping this whole block along with a few other low-SHAP features, but the result was inconclusive rather than a clear win. See [Model Development & Training](/Model_Development_&_Training).")
