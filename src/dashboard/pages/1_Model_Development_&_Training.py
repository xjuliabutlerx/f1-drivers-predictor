...
st.write("This project uses the same `fastf1` data source and 2018-2026 window as `f1-constructors-predictor`, downloaded and cleaned by a shared-shape but independent data pipeline (`src/data/`). The row grain here is `(Year, DriverId, Round)`, built only from rounds a driver actually raced rather than every calendar round like the constructors project. Unlike a constructor, a driver may miss a round whether it's a Did Not Start (DNS), injury, or a mid-season swap. `RoundsCompleted` (rounds actually raced) and `Round` (calendar round number) are genuinely different information here rather than being collinear, which is why both are kept as features.")
...
- Least Influential Features (per SHAP analysis, consistent across all 3 models): the individual `TeamId_*` one-hot columns, `HasQualifyingData`, and the DNF-cause breakdown features (`DriverFaultDNFRate`/`MechanicalDNFRate` and their per-round variants). These formed a leading candidate list for a future v2 to prune
- Model Architecture: 4 fully-connected layers. The first 2 use ReLU, Dropout, and BatchNorm; the 3rd uses ReLU and Dropout; the last is a plain linear output
""")
...
st.write("v2 kept v1's exact 57-feature set and dataset split methodology, and only changed the model architecture. Each change was tested one variable at a time, and always validated across all 3 season-holdout pairs before being trusted:")
st.markdown("""
1. **Widened hidden layers** (128/64/32 -> 256/128/64): a real, consistent improvement across all 3 pairs, so it was kept.
2. **ReLU -> GELU**: regressed rho on all 3 pairs, so it was reverted.
...
- **Driver age** (age-at-mid-season, from Ergast birthdate data): mixed. Worse on 2 pairs, notably better on 2024-2025 (+0.0106), and the most plausible candidate for a *real* small effect, but a single run per pair can't distinguish that from luck.
- **SHAP-pruned features** (dropping `HasQualifyingData` and the DNF-cause breakdown, meaning `DriverFaultDNFRate`/`MechanicalDNFRate` and their per-round counts, all flagged as the least-influential features by v1's SHAP analysis): also mixed, one pair up, two down. Deliberately did *not* drop the `TeamId_*` one-hot block despite it ranking low individually too, since one-hot dummies are mutually correlated by construction, so SHAP dilutes credit across all 10 of them the same way it diluted a correlated points cluster in an earlier (also-abandoned) pruning attempt; dropping the whole block risked losing real car-identity signal, not noise.
""")
...
st.write("Pairs are weighted by how close the two drivers' points totals are (`1.0 + alpha / (1.0 + points_gap_per_round)`), so tighter competition gets more training emphasis wherever it happens in the standings. This is unlike a fixed rank-band assumption, which doesn't hold for a ~20-24 driver field where title fights can be just as tight at the front as in the midfield.")
...
st.write("Traditional accuracy simply answers: *how often did the model predict the exact correct rank?* In a ranking task, that's misleading, since predicting a driver 5th instead of 6th gets penalized the same as predicting them 5th instead of 15th. It doesn't capture how close the model was when it was wrong.")
